...
def create_export_list(collection, exportHiddenObjects, useNamesFrom):
    ...
    for object in collection.objects:
        if object.type == "MESH" and (
            object.hide_viewport is False or exportHiddenObjects
        ):
            ...
            for vertex in bm.verts:
                if not vertex.is_manifold:
                    ...
                    error += [
                        (
                            {"ERROR"},
                            f"ALAMO - Non manifold geometry shadow mesh: {object.name}",
                        )
                    ]
                    break
...
def checkUV(object):  # throws error if object lacks UVs
    ...
    for material in object.data.materials:
        if (
            material.shaderList.shaderList == "MeshShadowVolume.fx"
            or material.shaderList.shaderList == "RSkinShadowVolume.fx"
        ):
            if len(object.data.materials) > 1:
                error += [
                    (
                        {"ERROR"},
                        f"ALAMO - Multiple materials on shadow volume: {object.name}; remove additional materials",
                    )
                ]
        if object.HasCollision:
            if len(object.data.materials) > 1:
                error += [
                    (
                        {"ERROR"},
                        f"ALAMO - Multiple submeshes/materials on collision mesh: {object.name}; remove additional materials",
                    )
                ]
    if (
        not object.data.uv_layers
    ):  # or material.shaderList.shaderList in settings.no_UV_Shaders:  #currently UVs are needed for everything but shadows
        ...
        if modifier.type == "ARMATURE":
            if modifier.object is None:
                error += [
                    (
                        {"ERROR"},
                        f"ALAMO - Armature modifier without selected skeleton on: {object.name}",
                    )
                ]
                break
                ...
                if modifier.object.name != activeSkeleton:
                    error += [
                        (
                            {"ERROR"},
                            f"ALAMO - Armature modifier skeleton doesn't match active skeleton on: {object.name}",
                        )
                    ]
                    break
    for constraint in object.constraints:
        if (
            ...
            error += [
                (
                    {"ERROR"},
                    f"ALAMO - Constraint doesn't match active skeleton on: {object.name}",
                )
            ]
            break
...
def checkFaceNumber(object):
    if len(object.data.polygons) > 65535:
        return [
            (
                {"ERROR"},
                f"ALAMO - {object.name} exceeds maximum face limit; split mesh into multiple objects",
            )
        ]
    return []
...
def checkAutosmooth(object):  # prints a warning if Autosmooth is used
    if object.data.use_auto_smooth:
        return [
            (
                {"ERROR"},
                f"ALAMO - {object.name} uses autosmooth, ingame shading might not match blender; use edgesplit instead",
            )
        ]
    return []
...
def checkTranslation(object):  # prints warning when translation is not default
    if object.location != mathutils.Vector(
        (0.0, 0.0, 0.0)
    ) or object.rotation_euler != mathutils.Euler((0.0, 0.0, 0.0), "XYZ"):
        return [
            (
                {"ERROR"},
                f"ALAMO - {object.name} is not aligned with the world origin; apply translation or bind to bone",
            )
        ]
    return []
...
def checkScale(object):  # prints warning when scale is not default
    if object.scale != mathutils.Vector((1.0, 1.0, 1.0)):
        return [
            ({"ERROR"}, f"ALAMO - {object.name} has non-identity scale. Apply scale.")
        ]
    return []
        ...
        for group in vertex.groups:
            if group.weight not in [0, 1]:
                return [
                    (
                        {"ERROR"},
                        f"ALAMO - Object {object.name} has improper vertex groups",
                    )
                ]
            total += group.weight
        if total not in [0, 1]:
            return [
                ({"ERROR"}, f"ALAMO - Object {object.name} has improper vertex groups")
            ]
    ...
    if type(object) is not type(None) and object.type == "MESH":
        ...
        if material is not None and material.shaderList.shaderList.find("RSkin") > -1:
            ...
            if len(set(used_groups)) > 23:
                return [
                    ({"ERROR"}, f"ALAMO - Object {object.name} has more than 23 bones.")
                ]
    return []
...
def checkTranslationArmature():  # prints warning when translation is not default
    ...
    if armature is not None and (
        ...
        return [
            (
                {"ERROR"},
                f"ALAMO - Armature {armature} is not aligned with the world origin; apply translation",
            )
        ]
    return []
                    ...
                    for keyframe in fcurve.keyframe_points:
                        ...
                        if this_keyframe == previous_keyframe:
                            local_errors += [
                                (
                                    {"WARNING"},
                                    f"ALAMO - {fcurve.group.name} has duplicate keyframe on frame {bpy.context.scene.frame_current}",
                                )
                            ]
