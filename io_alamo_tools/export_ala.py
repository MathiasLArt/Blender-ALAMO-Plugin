        ...
        while scene.frame_current <= animLength:  # iterate over every frame
            if pose.parent != None:
                translation_relative = (
                    pose.parent.matrix.inverted() @ pose.matrix
                ).decompose()[0]
            else:
    ...
    while scene.frame_current <= animLength:
        for name in translationList:
            ...
            chunk += struct.pack(
                "<H",
                int(
                    (translation_relative[0] - translationOffsetDict[pose.name][0])
                    / translationScaleDict[bone.name][0]
                ),
            )
            chunk += struct.pack(
                "<H",
                int(
                    (translation_relative[1] - translationOffsetDict[pose.name][1])
                    / translationScaleDict[bone.name][1]
                ),
            )
            chunk += struct.pack(
                "<H",
                int(
                    (translation_relative[2] - translationOffsetDict[pose.name][2])
                    / translationScaleDict[bone.name][2]
                ),
            )
...
def create_animation():
    ...
    for pose in armature.pose.bones:
        ...
        for curve in armature.animation_data.action.fcurves:
            ...
            if (
                curve.data_path.split('"')[1] == pose.name
                and curve.data_path.split('"')[2] == "].location"
            ):
                if not (pose.name in translationList):
                    translationList.append(
                        pose.name
                    )  # if list doesnt contaion bone name add it
            if (
                curve.data_path.split('"')[1] == pose.name
                and curve.data_path.split('"')[2] in settings.rotation_curve_name
            ):
    ...
    bpy.context.scene.frame_set(
        0
    )  # set scene to first frame, needed for default rotation/translation
    # add the bone data chunks for every bone
    for bone in armature.data.bones:
        if bone.name != "Root":
            chunk += create_bone_data(bone, translationList, rotationList, armature)

    chunk += create_translation_data(
        translationList, armature
    )  # add translation data chunk
...
def create_anim_info_chunk(armature):
    ...
    chunk += struct.pack("<I", len(armature.data.bones))  # add number of bones
    ...
    for pose in armature.pose.bones:  # iterate over every bone
        ...
        for curve in armature.animation_data.action.fcurves:
            ...
            if (
                curve.data_path.split('"')[1] == pose.name
                and curve.data_path.split('"')[2] == "].rotation_quaternion"
            ):
                rotationTracks += 1
...
def create_bone_data(bone, translationList, rotationList, armature):
    chunk = create_bone_animation_info_chunk(
        bone, translationList, rotationList, armature
    )
...
def create_bone_animation_info_chunk(bone, translationList, rotationList, armature):
    ...
    translationScale = calculateTranslationScale(
        bone, translationOffset, armature, translationList
    )
    ...
    if bone.name in translationList:
        translationIndex = (
            translationList.index(bone.name) * 3
        )  # if bone is in location list use index in list as index
    else:
    ...
    if bone.name in rotationList:
        rotationIndex = 4 * rotationList.index(
            bone.name
        )  # if bone is in rotation list use index in list as index
    else:
...
def create_visibility_chunk(armature, bone):
    ...
    for curve in armature.animation_data.action.fcurves:
        ...
        if parts[2] == "].proxyIsHiddenAnimation":
            if parts[1] == bone.name or (
                bone.parent is not None and bone.parent.name == parts[1]
            ):
                dataExists = True
                break
    ...
    scene.frame_set(0)
    while scene.frame_current <= animLength:
        if pose.proxyIsHiddenAnimation or (
            parentPose is not None and parentPose.proxyIsHiddenAnimation
        ):
            binary += "0"
        else:
            binary += "1"
    ...
    def exportAnimation(self, path):
        if os.access(path, os.W_OK) or not os.access(path, os.F_OK):
            ...
            with disable_exception_traceback():
                raise Exception(
                    f"ALAMO - EXPORT FAILED; can't write {os.path.split(path)[1]}"
                )
...
class ALA_Exporter(bpy.types.Operator):
    ...
    bl_idname = (
        "export_anim.ala"  # unique identifier for buttons and menu items to reference.
    )
    ...
    filepath: StringProperty(
        name="File Path",
        description="Filepath used for exporting the ALO file",
        maxlen=1024,
        default="",
    )
