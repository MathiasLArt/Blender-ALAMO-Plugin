...
def read_visibility_data(data, bone_Name):
    ...
    string = (
        string.replace("x", "").replace("b", "").replace("\\", "").replace("'", "")
    )  # .replace('<', '')
    binary = ""
    counter = 0
    ...
    while fps_counter < data.num_frames:
        ...
        while bone_counter < data.num_bones:
            ...
            if data.rotation_index[bone_counter] != -1:
                rotation_unpacked = data.rotation_data[
                    fps_counter * data.rotation_block_size
                    + data.rotation_index[bone_counter]
                ]
            else:
            ...
            if data.translation_index[bone_counter] != -1:
                ...
                t_packed = data.translation_data[
                    fps_counter * data.translation_block_size
                    + int(data.translation_index[bone_counter] / 3)
                ]  # translation index counts floats, data is stored as vector, thats why divide by 3
                for i in range(3):
            ...
            if (
                data.bone_name_list[data.bone_index[bone_counter] - 1]
                in armature.pose.bones
            ):
                pose = armature.pose.bones[
                    data.bone_name_list[bone_counter]
                ]  # pose bone of current bone
                bone = armature.data.bones[
                    data.bone_name_list[bone_counter]
                ]  # bone of current bone

                if pose.parent != None:
                    pose.matrix = (
                        pose.parent.matrix @ translationMatrix @ rotationMatrix
                    )
                else:
                    pose.matrix = translationMatrix @ rotationMatrix

                if (
                    data.rotation_index[bone_counter] != -1
                    or fps_counter == 0
                    or fps_counter == data.num_frames - 1
                ):
                ...
                if bone.name in data.visibilityDict:
                    pose.proxyIsHiddenAnimation = (
                        data.visibilityDict[pose.name][scene.frame_current] == "0"
                    )
...
class ALA_Importer(bpy.types.Operator):
    ...
    bl_idname = (
        "import_anim.ala"  # unique identifier for buttons and menu items to reference.
    )
    ...
    filepath: StringProperty(
        name="File Path",
        description="Filepathused for importing the ALA file",
        maxlen=1024,
        default="",
    )
